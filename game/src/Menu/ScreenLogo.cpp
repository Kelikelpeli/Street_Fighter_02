...
void ScreenLogoState::DrawScreen(void)
{
    ...
    {
        if ((framesCounter / 10) % 2) DrawRectangle(logoPositionX, logoPositionY, 16, 16, WHITE);
    }
    ...
    else if (state == 3)    // Draw "raylib" text-write animation + "powered by"
    {
        ...
        if (framesCounter > 20) DrawText("powered by", logoPositionX, logoPositionY - 27, 20, Fade(LIGHTGRAY, alpha));
    }
}
